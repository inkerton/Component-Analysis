...
const processSbom = async (composerFile, userId) => {
  // const json = JSON.parse(fileContent);
  // const components = await parseSbom(json);
  const packageDir = path.dirname(composerFile.path);
  try {
    const fileBuffer = fs.readFileSync(composerFile.path);

   
    const form = new FormData();
    form.append("composer_file", fileBuffer, composerFile.originalname);
    ...
    const { status, data } = await axiosInst.post(
      `${config.get("parserServer")}/php/composer-parser`,
      form
    );
    ...
    fs.unlinkSync(composerFile.path);
  }
};
