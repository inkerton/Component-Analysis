...
const processSbom = async (pomFile, userId) => {
  // const json = JSON.parse(fileContent);
  // const components = await parseSbom(json);
  const packageDir = path.dirname(pomFile.path);
  try {
    const fileBuffer = fs.readFileSync(pomFile.path);

   
    const form = new FormData();
    form.append("pom_file", fileBuffer, pomFile.originalname);
    ...
    const { status, data } = await axiosInst.post(
      `${config.get("parserServer")}/java/maven-parser`,
      form
    );
